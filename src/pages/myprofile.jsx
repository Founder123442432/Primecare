...
export default function Myprifile() {
  const [user] = useAuthState(auth);
  const bas = useNavigate();
      ...
      <section className="relative pt-40 pb-24">
        ...
        <div className="w-full max-w-7xl mx-auto px-6 md:px-8">
          ...
          <div className="flex max-sm:flex-wrap max-sm:justify-center items-center gap-4">
            <span className="rounded-full py-3 px-6 bg-stone-100 text-gray-700 font-semibold text-sm leading-6 transition-all duration-500 hover:bg-stone-200 hover:text-gray-900">
              <b>ID: </b> {user?.uid}
            </span>
            <span className="rounded-full py-3 px-6 bg-stone-100 text-gray-700 font-semibold text-sm leading-6 transition-all duration-500 hover:bg-stone-200 hover:text-gray-900">
              <b>Gender:</b>"..."
